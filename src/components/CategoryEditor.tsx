...
import type { BilingualText, Category, Room } from '../types';
...
interface FormState {
  catLabelEs: string; catLabelEn: string;
  nameEs: string; nameEn: string;
  shortNameEs: string; shortNameEn: string;
  blurbEs: string; blurbEn: string;
  rooms: Room[];
}

const EMPTY_BILINGUAL: BilingualText = { es: '', en: '' };
function emptyRoom(): Room {
  return { slug: '', img: '', thumb: '', area: '', name: { ...EMPTY_BILINGUAL }, tag: { ...EMPTY_BILINGUAL }, blurb: { ...EMPTY_BILINGUAL }, features: [] };
}

function toForm(cat: Category): FormState {
  return {
    catLabelEs: cat.catLabel.es, catLabelEn: cat.catLabel.en,
    nameEs: cat.name.es, nameEn: cat.name.en,
    shortNameEs: cat.shortName?.es ?? '', shortNameEn: cat.shortName?.en ?? '',
    blurbEs: cat.blurb?.es ?? '', blurbEn: cat.blurb?.en ?? '',
    rooms: cat.rooms ?? [],
  };
}
...
function toPatch(f: FormState): Record<string, unknown> {
  return {
    catLabel: { es: f.catLabelEs.trim(), en: f.catLabelEn.trim() },
    name: { es: f.nameEs.trim(), en: f.nameEn.trim() },
    shortName: { es: f.shortNameEs.trim(), en: f.shortNameEn.trim() },
    blurb: { es: f.blurbEs.trim(), en: f.blurbEn.trim() },
    rooms: f.rooms.map((r) => ({
      slug: r.slug,
      img: r.img,
      thumb: r.thumb,
...
export function CategoryEditor({ typeKey, category, onClose, onSaved }: {
  typeKey: string;
  category: Category;
  onClose: () => void;
  ...
  function addRoom() { setForm((prev) => ({ ...prev, rooms: [...prev.rooms, emptyRoom()] })); }
  function removeRoom(index: number) { setForm((prev) => ({ ...prev, rooms: prev.rooms.filter((_, i) => i !== index) })); }
  ...
  async function handleUpload(index: number, file: File) {
    ...
    try {
      ...
      updateRoom(index, { img: url, thumb: thumbUrl });
    } catch (err) {
  ...
  async function save() {
    ...
    setError(null);
    try {
      const updated = await api.updateCategory(typeKey, toPatch(form));
      onSaved(updated);
    } catch (err) {
    ...
    >
      ...
      >
        <div className="mb-5 flex items-start justify-between gap-3">
          <div>
            <div className="font-display text-xl font-semibold text-ink">{form.catLabelEs || typeKey}</div>
            <div className="mt-1 text-xs text-muted">{typeKey}</div>
          </div>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-3">
            <Field label="Nombre corto (ES)" value={form.catLabelEs} onChange={(e) => setForm((p) => ({ ...p, catLabelEs: e.target.value }))} placeholder="Amoblado Loft 1 Ambiente" />
            <Field label="Short label (EN)" value={form.catLabelEn} onChange={(e) => setForm((p) => ({ ...p, catLabelEn: e.target.value }))} />
            <Field label="Nombre completo (ES)" value={form.nameEs} onChange={(e) => setForm((p) => ({ ...p, nameEs: e.target.value }))} placeholder="Apartaestudio, 1 Ambiente" />
            <Field label="Full name (EN)" value={form.nameEn} onChange={(e) => setForm((p) => ({ ...p, nameEn: e.target.value }))} />
            <Field label="Nombre de pestaña (ES)" value={form.shortNameEs} onChange={(e) => setForm((p) => ({ ...p, shortNameEs: e.target.value }))} placeholder="1 Ambiente" />
            <Field label="Tab label (EN)" value={form.shortNameEn} onChange={(e) => setForm((p) => ({ ...p, shortNameEn: e.target.value }))} placeholder="1 Room" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <TextArea label="Descripción del modelo (ES)" rows={3} value={form.blurbEs} onChange={(e) => setForm((p) => ({ ...p, blurbEs: e.target.value }))} />
            <TextArea label="Model description (EN)" rows={3} value={form.blurbEn} onChange={(e) => setForm((p) => ({ ...p, blurbEn: e.target.value }))} />
          </div>

          <div className="border-t border-line pt-4">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-xs font-bold uppercase tracking-wide text-muted">Ambientes ({form.rooms.length})</h3>
            ...
            <div className="space-y-2">
              {form.rooms.map((room, i) => (
                <RoomEditor
                  ...
                  canMoveUp={i > 0}
                  canMoveDown={i < form.rooms.length - 1}
                />
              ))}
              {form.rooms.length === 0 && <p className="text-sm text-muted">Todavía no hay ambientes — agrega el primero arriba.</p>}
            </div>
          </div>
